...
export class MfaRepository extends BaseRepository {
  async getTotp(userId: number): Promise<MfaTotpRecord | undefined> {
    const row = await this.table("user_mfa_totp").select("*").where("user_id", userId).first();
  ...
  async putUnconfirmedTotp(userId: number, secretEnc: string, now: number): Promise<void> {
    await this.table("user_mfa_totp").where("user_id", userId).del();
    await this.table("user_mfa_totp").insert({
      user_id: userId,
  ...
  async confirmTotp(userId: number, now: number, step: number): Promise<boolean> {
    const updated = await this.table("user_mfa_totp")
  ...
  async replaceRecoveryCodes(userId: number, hashes: string[], now: number): Promise<void> {
    await this.table("user_mfa_recovery_codes").where("user_id", userId).del();
    if (hashes.length === 0) return;
    await this.table("user_mfa_recovery_codes").insert(
  ...
  async getUnusedRecoveryCodes(userId: number): Promise<MfaRecoveryCodeRecord[]> {
    const rows = (await this.table("user_mfa_recovery_codes")
      .select("*")
      .where("user_id", userId)
      .whereNull("used_at")
  ...
  async useRecoveryCode(id: number, now: number): Promise<boolean> {
    const updated = await this.table("user_mfa_recovery_codes")
      .where("id", id)
      .whereNull("used_at")
      .update({ used_at: now })
      .then((n) => Number(n));
  ...
  async countRecoveryCodes(userId: number): Promise<{ total: number; unused: number }> {
    const rows = (await this.table("user_mfa_recovery_codes").select("used_at").where("user_id", userId)) as Record<
      string,
      unknown
    >[];
    return {
  ...
  async getUserIdsWithConfirmedTotp(): Promise<number[]> {
    const rows = (await this.table("user_mfa_totp").select("user_id").whereNotNull("confirmed_at")) as Record<
      string,
      unknown
    >[];
